...
package fsu.gildedsky.gildedsky.portal;
...
public class SkyTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("sky_portal", Vec3i::compareTo, 300);
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(ModBlocks.SKY_PORTAL_BLOCK.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register("sky_portal", poiType);
	...
	public SkyTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	public Optional<BlockUtil.FoundRectangle> findPortalAround(BlockPos p_192986_, boolean p_192987_, WorldBorder p_192988_) {
		...
		Optional<PoiRecord> optional = poimanager.getInSquare((p_230634_)
				-> p_230634_.is(poi.unwrapKey().get()), p_192986_, i, PoiManager.Occupancy.ANY).filter((p_192981_)
				-> p_192988_.isWithinBounds(p_192981_.getPos())).sorted(Comparator.<PoiRecord>comparingDouble((p_192984_)
				-> p_192984_.getPos().distSqr(p_192986_)).thenComparingInt((p_192992_)
				-> p_192992_.getPos().getY())).filter((p_192990_)
				-> this.level.getBlockState(p_192990_.getPos()).hasProperty(BlockStateProperties.HORIZONTAL_AXIS)).findFirst();
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = ModBlocks.SKY_PORTAL_BLOCK.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = SkyPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(), entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return SkyPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()), entity.getDeltaMovement(), entity.getYRot(), entity.getXRot());
